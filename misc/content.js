const content = {};

content.images = [
  {
    image: "https://res.cloudinary.com/aadityac73/image/upload/v1606717532/assets/allImages/branding_m0x9lq.jpg",
    caption: "" 
  },
  {
    image: "https://res.cloudinary.com/aadityac73/image/upload/v1606717535/assets/allImages/img1_jg3tud.jpg",
    caption: "" 
  },
  {
    image: "https://res.cloudinary.com/aadityac73/image/upload/v1606717538/assets/allImages/img21_echbxn.jpg",
    caption: "" 
  },
  {
    image: "https://res.cloudinary.com/aadityac73/image/upload/v1606717539/assets/allImages/img23_zby5r7.jpg",
    caption: "" 
  },
  {
    image: "https://res.cloudinary.com/aadityac73/image/upload/v1606717539/assets/allImages/img25_chh2xk.jpg",
    caption: "" 
  },
  {
    image: "https://res.cloudinary.com/aadityac73/image/upload/v1606717539/assets/allImages/img26_woaeby.jpg",
    caption: "" 
  },
  {
    image: "https://res.cloudinary.com/aadityac73/image/upload/v1606717533/assets/allImages/flooring_csdxpz.jpg",
    caption: "" 
  },
  {
    image: "https://res.cloudinary.com/aadityac73/image/upload/v1606717533/assets/allImages/furniture1_cbm0e4.jpg",
    caption: "" 
  },
  {
    image: "https://res.cloudinary.com/aadityac73/image/upload/v1606717534/assets/allImages/furniture2_ygqnyi.jpg",
    caption: "" 
  },
  {
    image: "https://res.cloudinary.com/aadityac73/image/upload/v1606717534/assets/allImages/furniture3_eumjkb.jpg",
    caption: "" 
  },
  {
    image: "https://res.cloudinary.com/aadityac73/image/upload/v1606717534/assets/allImages/furniture4_ubkgm4.jpg",
    caption: "" 
  },
  {
    image: "https://res.cloudinary.com/aadityac73/image/upload/v1606717536/assets/allImages/img7_rgveso.jpg",
    caption: "" 
  },
  {
    image: "https://res.cloudinary.com/aadityac73/image/upload/v1606717534/assets/allImages/gym1_issng3.jpg",
    caption: "" 
  },
  {
    image: "https://res.cloudinary.com/aadityac73/image/upload/v1606717534/assets/allImages/gym2_zngn7m.jpg",
    caption: "" 
  },
  {
    image: "https://res.cloudinary.com/aadityac73/image/upload/v1606717553/assets/allImages/gym3_yzqojt.png",
    caption: "" 
  },
  {
    image: "https://res.cloudinary.com/aadityac73/image/upload/v1606717535/assets/allImages/gym4_ceceel.jpg",
    caption: "" 
  },
  {
    image: "https://res.cloudinary.com/aadityac73/image/upload/v1606717535/assets/allImages/image_by1ays.jpg",
    caption: "" 
  },
  {
    image: "https://res.cloudinary.com/aadityac73/image/upload/v1606717537/assets/allImages/img10_fngmjb.jpg",
    caption: "" 
  },
  {
    image: "https://res.cloudinary.com/aadityac73/image/upload/v1606717537/assets/allImages/img11_i84pon.jpg",
    caption: "" 
  },
  {
    image: "https://res.cloudinary.com/aadityac73/image/upload/v1606717537/assets/allImages/img12_b4ydis.jpg",
    caption: "" 
  },
  {
    image: "https://res.cloudinary.com/aadityac73/image/upload/v1606717537/assets/allImages/img13_mgensg.jpg",
    caption: "" 
  },
  {
    image: "https://res.cloudinary.com/aadityac73/image/upload/v1606717537/assets/allImages/img14_ixt7ia.jpg",
    caption: "" 
  },
  {
    image: "https://res.cloudinary.com/aadityac73/image/upload/v1606717537/assets/allImages/img15_qesshd.jpg",
    caption: "" 
  },
  {
    image: "https://res.cloudinary.com/aadityac73/image/upload/v1606717538/assets/allImages/img16_ivxebe.jpg",
    caption: "" 
  },
  {
    image: "https://res.cloudinary.com/aadityac73/image/upload/v1606717537/assets/allImages/img2_rsogcz.jpg",
    caption: "" 
  },
]

content.services = [
  {
    name: "Civil and Interior",
    class: "one",
    content: "We uniquely offer a 360° fully integrated design service that provides end-to-end solutions, which includes branding, architecture, interiors, civil work, sustainability and security. We have developed a unique client focus that creates and delivers profitable, sustainable and future-ready solutions along with our in-house technical specialists creating spaces that connect with the user both aesthetically and functionally.",
    images: [
      "https://res.cloudinary.com/aadityac73/image/upload/v1605965866/assets/Services/interior1_stv1y1.jpg",
      "https://res.cloudinary.com/aadityac73/image/upload/v1605965865/assets/Services/interior_arqgqx.jpg",
      "https://res.cloudinary.com/aadityac73/image/upload/v1605965865/assets/Services/interior2_mfowsi.jpg"
    ]
  },
  {
    name: "Furniture",
    class: "two",
    content: "Our team has specialized furniture designer with heart winning display of abilities with their modern designing. They are head and hand behind giving the service making office furniture ,lobby area, bank furniture ,conference room,dinning room.",
    images: [
      "https://res.cloudinary.com/aadityac73/image/upload/v1605965829/assets/Services/furniture3_mo89wr.jpg",
      "https://res.cloudinary.com/aadityac73/image/upload/v1605965829/assets/Services/furniture4_r6qq6t.jpg",
      "https://res.cloudinary.com/aadityac73/image/upload/v1605965267/assets/Services/furniture_lkiouz.jpg"
    ]
  },
  {
    name: "Electrical and Lighting Work",
    class: "three",
    content: "When you need electrician V R ENTERPRISES is here to help. Every minute care is taken to hide cable and wire and give a clean and elegant look.",
    images: [
      "https://res.cloudinary.com/aadityac73/image/upload/v1605964279/assets/Services/light_f8f7sk.jpg"
    ]
  },
  {
    name: "Painting and Finishing",
    class: "four",
    content: "The tem gives the various option of ENAMEL, LUSTER, PLASTIC PAINT, ROYAL, TEXTURE PAINT and Finish to client and based on choice of client give desired look.",
    images: ["https://res.cloudinary.com/aadityac73/image/upload/v1606717535/assets/allImages/image_by1ays.jpg"]
  },
  {
    name: "Wall Art Hand Painting",
    class: "five",
    content: "",
    images: ["https://res.cloudinary.com/aadityac73/image/upload/v1605964239/assets/Services/wall_zgnaq0.jpg"]
  },
  {
    name: "Fire Hydrant System",
    class: "six",
    content: "The team design keeping in consideration the amenities required to meet all emergency situation in particular organization the re hydrant system.",
    images: []
  },
  {
    name: "Modular Work",
    class: "seven",
    content: "The team based on client based on the modular requirement, make and design modular work which are in accordance to the specific need to please the client both in term of utility and looks.",
    images: [
      "https://res.cloudinary.com/aadityac73/image/upload/v1605966271/assets/Services/modular_ddts1f.jpg",
      "https://res.cloudinary.com/aadityac73/image/upload/v1605966271/assets/Services/modular1_o3toow.jpg"
    ]
  },
  {
    name: "HVAC Partitioning & Wall Paneling",
    class: "eight",
    content: "The team has expertise for modular panel for critical and non critical application and look after installation in all the offices and large industrial unit",
    images: []
  },
  {
    name: "Flooring",
    class: "nine",
    content: "Our team fixed all type of flooring vinly flooring, wooden flooring, Italian marble, vitrified flooring, etc. we suggest the flooring that will be best suitable to the particular area.",
    images: [
      "https://res.cloudinary.com/aadityac73/image/upload/v1605966306/assets/Services/floor2_t7gteh.jpg",
      "https://res.cloudinary.com/aadityac73/image/upload/v1605966306/assets/Services/floor3_zrctlt.jpg",
      "https://res.cloudinary.com/aadityac73/image/upload/v1605966306/assets/Services/flooring_to7n1f.jpg"
    ]
  },
  {
    name: "Branding",
    class: "ten",
    content: "",
    images: []
  }
];

content.clients= [
  {
    name: "Rare Rabbit",
    logo: "https://res.cloudinary.com/aadityac73/image/upload/v1605608131/assets/logos/rare_rabbit_wijkho.png",
    images: [
              "https://res.cloudinary.com/aadityac73/image/upload/v1605512889/assets/images/rr1_obesqn.png",
              "https://res.cloudinary.com/aadityac73/image/upload/v1605512889/assets/images/rr2_xo2oo4.png",
              "https://res.cloudinary.com/aadityac73/image/upload/v1605512889/assets/images/rr3_arj1ux.png"
            ]
  },
  {
    name: "Ujjivan Finance Bank",
    logo: "https://res.cloudinary.com/aadityac73/image/upload/v1605608132/assets/logos/bank_logo_srelir.jpg",
  },
  {
    name: "IIFL",
    logo: "https://res.cloudinary.com/aadityac73/image/upload/v1605608133/assets/logos/IIFL_wnlf3k.jpg",
    images: []
  },
  {
    name: "Muthoot Finance",
    logo: "https://res.cloudinary.com/aadityac73/image/upload/v1605608131/assets/logos/Muthoot_Finance_fchx6t.png",
    images: []
  },
  {
    name: "Huawei",
    logo: "https://res.cloudinary.com/aadityac73/image/upload/v1605608134/assets/logos/huawei_b9bfce.png",
    images: []
  },
  {
    name: "Allied Design Associates",
    logo: "https://res.cloudinary.com/aadityac73/image/upload/v1605608132/assets/logos/ada_unev1u.jpg",
    images: []
  },
  {
    name: "FIS",
    logo: "https://res.cloudinary.com/aadityac73/image/upload/v1605608132/assets/logos/fis_ty5yyc.png",
    images: []
  },
  {
    name: "Korum Mall",
    logo: "https://res.cloudinary.com/aadityac73/image/upload/v1605608131/assets/logos/korum_vjqmm0.png",
    images: []
  },
  {
    name: "Kailash Parbat Restaurant",
    logo: "https://res.cloudinary.com/aadityac73/image/upload/v1605608133/assets/logos/kailash-parbat_xyxaqy.jpg",
    images: []
  },
  {
    name: "Pernod Ricard",
    logo: "https://res.cloudinary.com/aadityac73/image/upload/v1605608131/assets/logos/Pernod-Ricard_edfawl.png",
    images: []
  },
  {
    name: "Royal Stag",
    logo: "https://res.cloudinary.com/aadityac73/image/upload/v1605608131/assets/logos/royal-stag_2_hypi1j.jpg",
    images: []
  },
  {
    name: "Blenders Pride",
    logo: "https://res.cloudinary.com/aadityac73/image/upload/v1605608132/assets/logos/bleders_logo_lmngxg.jpg",
    images: []
  },
];

module.exports = content;